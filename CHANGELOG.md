...
- Fixed a rare, hard-to-reproduce bug where the editor's "currently running line" highlight/scroll could land on a stale line (sometimes leaving a black gap at the end of a fast job, sometimes just a momentarily-wrong highlighted line) - every command-completed event triggers its own independent status fetch, and near the end of a fast job these can resolve out of order; whichever happened to come back last was winning even if it wasn't actually the most recent one.
